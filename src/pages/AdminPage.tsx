...
const AdminPage = () => {
  ...
  const loadData = async () => {
    setLoadingData(true);
    const [pending, all, reported] = await Promise.all([
      fetchPendingGames(),
      fetchAllGames(),
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3 mb-8">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <p className="text-3xl font-bold text-primary mt-2">{allGames.length}</p>
        </div>
        <div className="rounded-xl border border-border bg-card p-6">
          <h3 className="font-display text-lg font-bold">Reported Comments</h3>
          <p className="text-3xl font-bold text-destructive mt-2">{reports.length}</p>
        </div>
      </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex gap-1 mt-1">
                      ...
                      {game.tags.slice(0, 3).map(t => <Badge key={t} variant="outline" className="text-xs">{t}</Badge>)}
                    </div>
                    {game.description && <p className="text-xs text-muted-foreground mt-1 line-clamp-1">{game.description}</p>}
                  </div>
                </div>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      By: {report.comments?.profiles?.username || 'Unknown'}
                    </p>
                    <p className="text-sm mt-1 rounded bg-secondary/50 p-2">{report.comments?.text || 'Comment deleted'}</p>
              ...
              {allGames.map(game => (
                <div key={game.id} className="flex items-center justify-between rounded-lg border border-border bg-card p-3">
                  <div className="flex items-center gap-3 min-w-0">
                    {game.image && <img src={game.image} alt={game.title} className="h-10 w-10 rounded object-cover flex-shrink-0" />}
